...
window.toggleLike = async function (btn) {
  if (btn.dataset.guest) { showToast('Sign in to like posts!', 'error'); window.location='/auth/login'; return; }
...
window.toggleSave = async function (btn) {
  if (btn.dataset.guest) { showToast('Sign in to save posts!', 'error'); window.location='/auth/login'; return; }
  const postId = btn.dataset.post;
  try {
    ...
    if (data.ok) {
      ...
      btn.lastChild.textContent = data.saved ? 'Saved' : 'Save';
      showToast(data.saved ? '🔖 Saved!' : 'Removed from bookmarks', 'success');
    }
  } catch (e) { showToast('Something went wrong', 'error'); }
};
...
window.openQuizTeaser = function (btn, postId) {
  ...
  if (!box) return;
  if (box.style.display === 'none') { box.style.display = 'block'; btn.style.display = 'none'; }
};
...
window.switchMode = function (mode) {
  ...
  if (!normal) return;
  if (mode === 'normal') {
    normal.style.display = 'block'; if (eli5) eli5.style.display = 'none';
    if (btnN) { btnN.classList.add('active'); } if (btnE) { btnE.classList.remove('active'); }
  } else {
    if (eli5) eli5.style.display = 'block'; normal.style.display = 'none';
    if (btnE) { btnE.classList.add('active'); } if (btnN) { btnN.classList.remove('active'); }
  }
};
...
window.markBest = async function (btn) {
  const id = btn.dataset.id;
  try {
    const res  = await fetch(`/api/comments/${id}/best`, { method: 'POST' });
    const data = await res.json();
    if (data.ok) { showToast('Marked as best answer! +25 XP', 'success'); location.reload(); }
...
window.switchTab = function (btn, tab) {
  ...
  if (tab === 'quiz')    document.getElementById('tab-quiz').style.display = 'flex';
...
window.updatePreview = function () {
  ...
  if (pb && body)  pb.innerHTML   = (body.value || 'Start typing to see preview…').replace(/\n/g,'<br>');
};

/* ── Quiz builder: add/remove questions ── */
let qCount = 1;
...
(function () {
  ...
  function go(delta) { window.microGo(delta); }
  ...
  mcCard.addEventListener('click', () => { window.microFlip(); });

  // Drag support
  mcCard.addEventListener('mousedown', e => { startX = e.clientX; dragging = true; });
  ...
  // Touch support
  mcCard.addEventListener('touchstart', e => { startX = e.touches[0].clientX; }, { passive: true });
  mcCard.addEventListener('touchend', e => {
...
window.showToast = function (msg, type) {
  ...
  t.style.cssText = 'position:fixed;top:70px;right:24px;z-index:500;padding:14px 20px;border-radius:12px;font-size:14px;display:flex;align-items:center;gap:10px;min-width:260px;animation:slideIn .3s ease';
  t.textContent = msg;
